...
if FSMB_game=="classic" or FSMB_game=="bfa" then 
...
--
FSMB_toonlist={[1]="Ammbphreak",[2]="Bmmbphreak",[3]="Cmmbphreak",[4]="Dmmbphreak",[5]="Emmbphreak",[6]="Fmmbphreak",[7]="Gmmbphreak",[8]="Hmmbphreak"}
FSMB_invitelist={[1]="Ammbphreak-tichondrius",[2]="Bmmbphreak-taerar",[3]="Cmmbphreak-blackhand",[4]="Dmmbphreak-blackmoore",[5]="Emmbphreak-echsenkessel",[6]="Fmmbphreak-lordaeron",[7]="Gmmbphreak-malganis",[8]="Hmmbphreak-antonidas"}
FSMB_tank=""
FSMB_clothto="Vaj"
FSMB_tradeopen=nil
FSMB_nomacros=nil
...
if FSMB_game=="bfa" then FSMB_myint["PALADIN"]=rebuke end
...
function init()
	...
	if FSMB_game=="bfa" then
		...
		elseif myClass=="SHAMAN" then
			if myspec==3 then myspec="RESTORATION"
	...
	clearmacros()
	if FSMB_game~="bfa" and FSMB_game~="classic" then personal=1 end
	if FSMB_game~="bfa" and FSMB_game~="classic" and myClass=="HUNTER" then hunterpersonal=1 end
	...
	local macroId = CreateMacroFS("focus_fs", "Ability_HUNTER_pathfinding", "/run focusme()" , hunterpersonal);
	PickupMacro(macroId)
	PlaceAction(43)
	ClearCursor()
	local slot=32
	...
	if myClass=="MONK" or myClass=="SHAMAN" or myClass=="PRIEST" or myClass=="PALADIN" or myClass=="DRUID" then
		...
		for i=1,TableLength(FSMB_toonlist) do
			...
			if FSMB_game=="wotlk" and myClass=="SHAMAN" then
				if FSMB_toonlist[i]==myname then
					heallist="\n/cast "..natureSwift.."\n/castsequence [@player] reset=combat/30 "..riptide..","..healingWave..","..lessHealingWave..","..lessHealingWave..","..lessHealingWave..","..lessHealingWave..","..lessHealingWave..","..lessHealingWave..","..lessHealingWave..","..lessHealingWave.."\n/cast "..tidalForce
				else
					heallist="\n/target "..FSMB_toonlist[i].."\n/castsequence [nomod] reset=combat/10 "..riptide..","..lessHealingWave..","..lessHealingWave..","..lessHealingWave..","..lessHealingWave..","..lessHealingWave..","..lessHealingWave..","..lessHealingWave.."\n/cast "..tidalForce
				end
	...
	if not FSMB_nomacros then
		...
		if myClass=="WARRIOR" and IsTank(myname) then
			...
			if RangedWeaponType() and SpellExists(shootGun) then
				GrabSpell(shootGun)
				PlaceAction(61)
			elseif RangedWeaponType() and SpellExists(shootCrossbow) then
				GrabSpell(shootCrossbow)
				PlaceAction(61)
			elseif SpellExists(shootBow) then
				GrabSpell(shootBow)
				PlaceAction(61)
			end
		end
		...
		if FSMB_game~="bfa" and myClass=="SHAMAN" and FindInTable(FSMB_healerlist,myname) then
		...
		if FSMB_game~="bfa" and myClass=="PALADIN" then
			if FindInTable(FSMB_healerlist,myname) then
		...
		if FSMB_tank==myname or (FSMB_game=="bfa" and (myClass=="HUNTER" or myClass=="WARRIOR" or myClass=="PALADIN")) or (FSMB_game~="bfa" and (myClass=="HUNTER" or myClass=="WARRIOR" or myClass=="ROGUE" or myClass=="SHAMAN" or myClass=="PALADIN" )) then
		...
		PickupMacro(index)
		PlaceAction(1)
		if myClass=="DRUID" or FSMB_game~="bfa" then
			PickupMacro(index)
			PlaceAction(13)
			PickupMacro(index)
			PlaceAction(73)
		...
		if FSMB_game~="bfa" and myClass=="HUNTER" then 
		...
		if myClass=="SHAMAN" then 
			if FSMB_game=="bfa" then
				index=CreateMacroFS("sheal_fs","INV_Misc_Questionmark","/castsequence [@player] reset=90/combat "..astralShift..","..shamanHeal..","..shamanHeal..","..shamanHeal..","..shamanHeal..","..shamanHeal..","..shamanHeal..","..shamanHeal..","..shamanHeal..","..shamanHeal..","..shamanHeal..","..shamanHeal..","..shamanHeal,nil)
			else
				index=CreateMacroFS("sheal_fs","INV_Misc_Questionmark","/castsequence [@player] reset=90/combat "..shamanHeal..","..shamanHeal..","..shamanHeal..","..shamanHeal..","..shamanHeal..","..shamanHeal..","..shamanHeal..","..shamanHeal..","..shamanHeal..","..shamanHeal..","..shamanHeal..","..shamanHeal,nil)
			end
		elseif myClass=="WARRIOR" then 
			if victoryRush and enragedRegen then 
				index=CreateMacroFS("sheal_fs","INV_Misc_Questionmark","/castrandom "..enragedRegen..","..victoryRush,nil)
			end
			if victoryRush then 
				index=CreateMacroFS("sheal_fs","INV_Misc_Questionmark","/cast "..victoryRush,nil)
			end
		elseif myClass=="MONK" then 
			index=CreateMacroFS("sheal_fs","INV_Misc_Questionmark","/castsequence reset=combat/90 "..touchOfKarma..","..dampenHarm,nil)
		elseif myClass=="DRUID" and FSMB_game=="wotlk" and IsFeral() then
			index=CreateMacroFS("sheal_fs","INV_Misc_Questionmark","/click sdf_BEAR_TANK_SHEAL",nil)
		elseif myClass=="DEATHKNIGHT" and FSMB_game=="wotlk" then
			index=CreateMacroFS("sheal_fs","INV_Misc_Questionmark","/click sdf_BLOOD_SHEAL",nil)
		elseif FSMB_selfheal[myClass] then
			index=CreateMacroFS("sheal_fs","INV_Misc_Questionmark","/cast [target=player] "..FSMB_selfheal[myClass],nil)
		end
		PickupMacro(index)
		PlaceAction(6)
		if myClass=="DRUID" or FSMB_game~="bfa" then
		...
		PlaceAction(31)
		ClearCursor()
		index=CreateMacroFS("leave_fs","spell_nature_purge","/script LeaveParty()",personal)
		PickupMacro(index)
		PlaceAction(30)
		ClearCursor()
		if FSMB_game=="wotlk" then
	...
	if myClass=="HUNTER" and FSMB_game~="bfa" then
	...
	if FSMB_game=="bfa" or FSMB_game=="classic" then 
...
--print("arg1:", arg1, "| arg2:", arg2)
	...
	elseif ((FSMB_game~="classic" and FSMB_game~="bfa") and event == "CHAT_MSG_ADDON") then
		...
		if arg1=="FSMB_FIND" then
			...
			FSMB_Find(item)
		elseif arg1==FSMB_RAID and arg2=="focusme" and arg4~=UnitName("player") then
			print(FSMB_RAID.." Focusing " .. arg4)
			FSMB_raidleader=arg4
		end
...
function Build_Spell_Array()
	...
	while true do
		...
		spellTable[spell] = {spellID, spellName, spellRank}
		spell = spell + 1
	end
	DEFAULT_CHAT_FRAME:AddMessage(C_Yellow .. "Spell Database Built!!")
end
function IAmFocus()
...
function follow()
	...
	if not IAmFocus() and FSMB_raidleader and IsAltKeyDown() then
...
["WARLOCK"]={"Conjured Mountain Spring Water",}, ["HUNTER"]={"Conjured Mountain Spring Water","Roasted Quail", "Jagged Arrow", "Accurate Slugs",}, ["SHAMAN"]={"Conjured Mountain Spring Water","Ankh",}, ["MAGE"]={"Conjured Mountain Spring Water","Arcane Powder","Rune of Portals"}, ["DRUID"]={"Conjured Mountain Spring Water", "Wild Thornroot", "Ironwood Seed",}, ["PALADIN"]={"Conjured Mountain Spring Water","Symbol of Kings",}, ["PRIEST"]={"Conjured Mountain Spring Water","Sacred Candle",}, ["ROGUE"]={"Flash Powder",}, ["DEATHKNIGHT"]={"Corpse Dust",}, }
	limits={["Symbol of Kings"]={600,100},["Corpse Dust"]={200,1},["Conjured Mountain Spring Water"]={800,1},["Roasted Quail"]={320,5},["Jagged Arrow"]={16,200},["Accurate Slugs"]={16,200},["Flash Powder"]={400,1},["Wild Thornroot"]={320,1},["Ironwood Seed"]={60,1},["Sacred Candle"]={640,1},["Ankh"]={30,1},["Arcane Powder"]={320,1},["Rune of Portals"]={80,1}}
