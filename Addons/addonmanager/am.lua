	...
	local checkbutton = CreateFrame("CheckButton", index, content,"UICheckButtonTemplate");
		...
		local checked = checkbutton:GetChecked()
		 ...
		 if checked == true then 
			 ...
			 print(am.cached_lang[1]..title..am.cached_lang[3])
		 elseif checked  ~= true then
			  DisableAddOn(title)

			print(am.cached_lang[2]..title..am.cached_lang[3])
		  end
...
function am:ShowAddonTooltip(title, notes, enabled, owner, author, version, dep, lod)
	...
	if dep ~= nil then
		GameTooltip:AddLine(" ");
		GameTooltip:AddLine(am.cached_lang[4]..dep);
	end
	
	GameTooltip:AddLine(" ");
	if lod ~= false then
			GameTooltip:AddLine("|cff69ccf0Load on Demand : |r"..am.cached_lang[12]);
	else
			GameTooltip:AddLine("|cff69ccf0Load on Demand : |r"..am.cached_lang[13]);
	end
...
function am:Toggle()
	...
	if am.isshown  == true then
		...
		if am.firststart == true then
			...
			am:CreateButton(nil, "CENTER", 100, -160, am.cached_lang[5],  EnableAllAddOns)
			am:CreateButton(nil, "CENTER", -100, -160, am.cached_lang[6],  DisableAllAddOns)
			am:CreateObjectButton(nil, "TOPRIGHT", 6, 10, "Interface\\BUTTONS\\UI-Panel-MinimizeButton-Up.png", 32, 32,  Hide)
			am:CreateButton(nil, "LEFT",  0, -160, am.cached_lang[7], GetMemoryUsage)
			am:DrawGUI()
			am:CreateFont(nil, "CENTER", 0, 130, "GameFontNormal", "CENTER", am.NumAddons.." Addons ("..(am.NumActivatedAddons-1)..am.cached_lang[8]..(am.NumAddons - am.NumActivatedAddons)..am.cached_lang[9]..", 1 "..am.cached_lang[10]..")")
			am.isshown = true
			am.firststart = false
		else
			bg:Show()
...
collectgarbage("collect")
	...
	GameTooltip:AddLine(am.cached_lang[7])
	GameTooltip:AddLine(" ")
	local g_memory = 0
...
function am:LoadTextByLang()
	...
	if location == "deDE" then
		...
		for i = 1, 13 do
			am.cached_lang[i] = Addonmanager.german[i]
		end
	elseif location ~= "deDE" then
		for i = 1, 13 do
			am.cached_lang[i] = Addonmanager.english[i]
		end
	end
end
