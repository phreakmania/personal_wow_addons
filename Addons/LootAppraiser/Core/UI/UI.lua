...
function UI.RefreshStatusText()
    if private.MAIN_UI ~= nil then
        ...
        local preparedText = "Filter: " .. LA.CONST.QUALITY_FILTER[tostring(LA.GetFromDb("general", "qualityFilter"))] 						-- filter
        preparedText = preparedText .. " - GAT: |cffffffff" .. LA.GetFromDb("general", "goldAlertThreshold") .. "|cffffd100g|r"				-- gat
        preparedText = preparedText .. " - Source: |cffffffff" .. priceSourceAsText or tostring(LA.db.profile.pricesource.source) .. "|r" 	-- price source

        --LA.private.MAIN_UI:SetStatusText(preparedText)
        local statustextUI = private.MAIN_UI:GetUserData("data_statustext")
        statustextUI:SetText(preparedText)
    end
end
...
function UI.RefreshUIs()
    ...
    if LA.GetFromDb("display", "showCurrencyLooted") and currencyLootedUI then
...
function UI.UpdateLastNoteworthyItemUI(itemLink, quantity, formattedValue)
    if private.LAST_NOTEWOTHYITEM_UI then
